...
def configuracion_sla(request, sla_id):
    ...
    TICKET.objects.all().update(sla_aplicado=sla)
    messages.success(request, f"El SLA '{sla.nombre_sla}' ha sido aplicado a todos los tickets.")
...
class crear_slaView(CreateView):
    model = SLA
    fields = ['nombre_sla', 'tiempo_respuesta', 'tiempo_resolucion']
...
def form_valid(self, form):
    form.instance.id_usuario_solicitante = self.request.user
    sla_por_defecto = SLA.objects.filter(nombre_sla="SLA aplicado").first()
    form.instance.sla_aplicado = sla_por_defecto
...
def pagLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            if user.last_name == 'Administrador':
